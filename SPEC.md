...
| Offline sync conflict logging | `sync_conflict:{user_id}` | 30 / 60 min (added in M6/M8: `log_sync_conflict`, wasn't in the original registry) |
...
- [x] Rate-limit sweep: every §6 bucket has a test that trips it and asserts 429 + RATE_LIMIT_TRIGGERED log
- [x] Security pass: no service key client-side (bundle grep), no PII/tokens in logs, error messages toast-safe, storage prefixes locked
- [x] a11y pass: contrast per brand table (coral never body-size text), focus-visible everywhere, reduced-motion verified, 44px targets
...
- [x] Forward-compat audit: grep confirms org access only via `getCurrentOrgId()`/`app_user_org_id()`; deviations fixed or documented
...
- **Security pass, done as an audit against the actual build output and source, not by inspection alone:** grepped `pnpm build`'s output — `SUPABASE_SERVICE_ROLE_KEY` appears only inside `.next/server/chunks/*` (the `/api/admin/*` route handlers' `createAdminClient()`), never once in `.next/static/*` (what actually ships to the browser). Searched every `console.*` call and every SQL `RAISE NOTICE`/`RAISE LOG` (as opposed to the sanctioned `RAISE EXCEPTION` error-code pattern) across `app/`, `components/`, `lib/`, `supabase/migrations/*.sql`, and `supabase/functions/` for JWTs, session tokens, passwords, password hashes, MFA backup codes/secrets, or the service-role key — zero call sites found in that scope at all (not zero *findings* — zero logging statements exist there, full stop). `scripts/seed-first-admin.ts` does print a plaintext bootstrap password to stdout, but that's a manual-run-only CLI tool never wired into deploy (CLAUDE.md), the password is meant to be read once and shared out-of-band by the operator, and it isn't the "password hash" CLAUDE.md's "never log" list actually names — left as-is, flagged for a second look only if seed output is ever piped to a persistent log or CI artifact.
- **Error-message toast-safety is enforced by the type system, not just eyeballed:** `lib/errorCodes.ts`'s `TOAST_MESSAGES` is typed `Record<ErrorCode, string>`, so TypeScript itself refuses to compile if any error code is missing a generic, non-technical message — every message was re-read for stack traces/SQL/internal IDs and found clean, and `toastMessageFor()`'s fallback ("Something went wrong. Please try again.") catches any code that somehow isn't in the registry, so a raw internal error can never reach a toast.
- **A real gap found and fixed by the audit, not just confirmed-clean:** `get_avatar_upload_url`'s rate-limit block never inserted a `RATE_LIMIT_TRIGGERED` audit row — the Edge Function already duplicates `private.check_rate_limit()`'s counting logic (PostgREST can't reach `private`, per the earlier M7 note), but the audit-log insert on block had been missed when that duplication was written. Fixed with a direct `AUD_SystemLog` insert mirroring `private.log_audit_event()`'s exact column shape. Still can't be exercised end-to-end in this sandbox (`supabase functions serve` doesn't run here — same constraint as the rest of the avatar upload flow), so this is a code-level fix verified by reading, not a passing test; flagged in §11 alongside the Edge Function's other untested-in-sandbox pieces.
- **Storage prefix locking re-verified, not re-tested from scratch:** `tests/rls/avatar.test.ts` (M7) already proves own-prefix upload succeeds and another user's prefix is blocked; re-read here to confirm it's still current against the `avatars` bucket policies, and the new `backups` bucket (previous note) extends the same "default-deny, service-role-only" pattern to a bucket with zero policies at all rather than prefix-scoped ones.
- **Rate-limit sweep found two real bugs, not hypotheticals — both fixed, not just flagged.** Writing a trip test for each §6 bucket (`tests/rls/rateLimitSweep.test.ts`, plus a missing audit-row assertion added to the pre-existing `login`/`report_export` trip tests) surfaced that `RATE_LIMIT_TRIGGERED` had never actually been persisted for almost any bucket: every guarded RPC does `perform private.log_audit_event(...); raise exception 'ERR_RATE_LIMITED';` in the same call, and PostgREST wraps each RPC call in one transaction — an uncaught `RAISE EXCEPTION` rolls back everything done earlier in that same transaction, including the audit insert a moment before. Confirmed directly with a throwaway probe function (insert, then raise, in one call: the insert never persisted) before writing any fix. Worse, the exact same rollback exposure applies to `private.check_rate_limit()`'s own **counting** insert, not just the audit log — `accept_invitation`'s trip test caught this directly: every attempt used a bogus token, which always raises `INVITATION_NOT_FOUND` *after* the rate check passes, so the counter's own insert was rolled back on every single call and 11 attempts never tripped the 10/10min limit at all. That's a real gap, not a cosmetic one: an attacker whose guesses reliably fail validation after the counter increment (token brute-forcing being the obvious case) gets effectively unlimited attempts.
- **The fix (`supabase/migrations/20260828000001_m8_rate_limit_audit_fix.sql`): both inserts now go through a real autonomous transaction via `dblink`**, the standard (and only, without a new extension) way to make a Postgres insert survive the caller's own transaction aborting. A dedicated `rate_limit_writer` role — `INSERT`-only on `RTL_RateLimitEvent`/`AUD_SystemLog` via `BYPASSRLS`, since neither table has an insert policy for any client-facing role, never granted to `anon`/`authenticated`, unreachable via PostgREST — opens a second loopback connection through `private.__autonomous_insert()`, a shared helper both `check_rate_limit()`'s counting insert and `log_rate_limit_breach()`'s audit insert now call. Best-effort by design: any failure in this path (missing credential, connection failure) is swallowed silently rather than raised, because audit/counting plumbing must never fail the caller's actual, already-decided request.
- **Two more things found and fixed only by testing the connection for real, not by reasoning about it:** (1) a hardcoded `host=127.0.0.1` failed even for a superuser caller with "password or GSSAPI delegated credentials required" — dblink refuses a supplied password when pg_hba.conf would authenticate that address via a method that ignores it, and this stack's local dev config trusts `127.0.0.1` outright as a convenience; switched to `inet_server_addr()` (whatever address the *current* connection actually came in on — for a real PostgREST-driven call, always a real routable address, confirmed directly: PostgREST connects via a Docker service hostname resolving to a real internal IP, never bare loopback) and confirmed the identical insert succeeds immediately once routed off `127.0.0.1`. (2) `inet_server_addr()::text` returns CIDR notation (`172.18.0.10/32`), which libpq's `host=` parameter rejects outright — fixed with `host(inet_server_addr())` to get the bare address.
- **`alter database ... set app.*` was tried first for storing the writer role's password and failed** ("permission denied to set parameter") — confirmed directly, not assumed: Supabase deliberately narrows the `postgres` role below true superuser even in this local dev image, matching the hosted platform's security model. Switched to a `private.__internal_secret` table instead (same revoke-from-every-client-role pattern as every other internal-only object in this schema), which needs no such privilege.
- **Real, measured perf cost, not asserted negligible:** the autonomous-insert path adds a genuine loopback connection per guarded RPC call — measured directly at ~15–24ms per call locally (vs. ~0.1–0.5ms for a trivial query with no dblink involved), on every call that passes the rate check, not just breaches, since there's no way to know in advance whether the rest of the function will succeed or fail. Against SPEC's punch p95 target (<500ms, measured from a real Singapore-hosted project), this is a small fraction of budget with real headroom, but it's a genuine cost of the correctness fix, not free — re-measure against a real deployed environment in the M8 perf-pass task rather than assuming the local number translates directly.
- **Discovered in passing:** `log_sync_conflict` (M6) has its own `sync_conflict:{user_id}` 30/60min bucket that was never added to SPEC §6's table — added there now, alongside the pre-existing note that `report_export`'s live code uses 20/1hr against the table's documented 30/60min (a known, already-flagged deviation, left as-is here).
- **Caveat carried forward, not resolved:** this was verified end-to-end against local Supabase's dev pg_hba.conf. A real hosted Supabase project's internal network/auth posture for this kind of Postgres-to-itself loopback hasn't been confirmed from this sandbox — the design (password auth over whatever address the connection actually arrived on, never a hardcoded loopback literal) should carry over, since every pg_hba.conf profile must support scram-sha-256 for pgbouncer/PostgREST to function at all, but this needs a real pass against a deployed project before launch. Added as a new §11 item.
- **a11y pass found and fixed four coral-as-body-text violations, not just a contrast reading.** `--tint` on plain text (`app/login/page.module.css` `.forgotLink`, `app/mfa/page.module.css` `.toggle`, `app/forgot-password/page.module.css` `.backLink`, `AdminNav.module.css` `.active`) computes to ≈2.77:1 against `--bg-elevated` white in light mode — fails WCAG AA outright — but the more direct problem is that SPEC's own brand table (§9) scopes `--tint` to "the ONLY accent; ONE filled-tint element per view," and plain text isn't a filled element at all. Fixed the three plain links to `--label-secondary` (the existing secondary-text convention used everywhere else in the app) and `AdminNav`'s active-tab text to `--label-primary`, keeping its `border-bottom: 2px solid var(--tint)` as the one filled-tint indicator for that view.
- **`--label-secondary` (`#7F8C8D` on white ≈3.48:1) itself falls short of WCAG AA for normal-size text and is used extensively at footnote/caption sizes app-wide — left as-is, not silently.** This hex is a literal value from SPEC's own brand table (§9, Brand Guidelines v1.1), the same category of fixed constraint as the wordmark artwork CLAUDE.md says never to re-typeset — not a component-level choice this pass is positioned to redesign. Flagged as a new §11 item for design sign-off rather than overridden unilaterally; dark mode's `#9AA5AE` value passes cleanly (≈6.2:1), so this is light-mode-only.
- **44px hit-target gaps fixed in three places the shared `Button`/`TextField`/`select` primitives don't cover:** `AppHeader`'s primary nav links (`Home`/`Timesheet`/`Requests`/`Dashboard`/`Admin`/`Profile` — bare `<Link>`s with no sizing, present on every authenticated page) now get a shared `.navLink` class (`flex`, `align-items: center`, `min-height: var(--hit-target-min)`); `AdminNav`'s sub-nav links (`padding: 4px 0` only, ≈23px total) get the same treatment inline; the MFA-enrollment backup-codes acknowledgment checkbox row (`app/mfa/enroll/page.module.css` `.checkboxRow`) gains `min-height: var(--hit-target-min)`. All three were confirmed correct pre-existing elsewhere (`NotificationBell`'s icon button, every form `select`/note-input, the shared `Button`) — the gaps were specifically bare `<Link>`/`<label>` elements outside the primitive system.
- **focus-visible and prefers-reduced-motion were both already fully compliant — confirmed by audit, not by assumption.** `app/globals.css` has one global `:focus-visible` rule covering every interactive element (no component relies on unstyled default, no stray `outline: none` found anywhere), and one global `@media (prefers-reduced-motion: reduce)` block zeroing animation/transition durations plus `scroll-behavior`, backed by redundant per-component `animation: none` overrides on every actually-animated component (`Sheet`, `NotificationBell`). No fix needed for either.
- **Forward-compat audit found `getCurrentOrgId()` was a dead export** — defined in `lib/org.ts` since M0, called nowhere. Four pages (`app/dashboard/page.tsx`, `app/timesheet/page.tsx`, `app/admin/work-arrangements/page.tsx`, `app/admin/org-settings/page.tsx`) each independently reimplemented the identical `select organization_id from "MST_User" where id = auth.uid()` query inline instead. No live bug today (same table, same predicate, same result) — but exactly the single-choke-point violation forward-compat rule #2 exists to prevent: when Phase 1 swaps `getCurrentOrgId()`'s body for subdomain resolution, these four call sites would silently keep resolving by signed-in user instead. Fixed by replacing all four with `await getCurrentOrgId()` (wrapped in try/catch where the page's existing behavior on a missing profile was to return `null` rather than error, matching each page's own prior fallback shape).
- **Everything else in the forward-compat audit was already compliant, confirmed by reading every RLS policy and RPC that touches `organization_id`, not by spot-check.** All 26+ RLS policies plus every RPC derive org scope via `private.app_user_org_id()` or the equivalent `auth.uid()`-driven `MST_User` lookup; no public RPC accepts a client-supplied `p_organization_id` (the parameter only exists on `private`-schema helpers fed by a caller-derived value); the 50-user-limit trigger is correctly org-scoped with only its numeric cap as the documented deliberate constant; the three pg_cron jobs correctly iterate rows across all orgs with no org filter on their driving queries (the right shape for a background job, not a single-org assumption) and carry each row's own `organization_id` downstream into every audit/notification insert.
...
19. **The M8 backup workflow's restore drill verified the `public`-schema portion of a restore against a real, isolated Postgres instance, but not the `auth`-schema portion** (M8) — the drill's own bare-Postgres test container never ran Supabase's actual Auth (GoTrue) service, so its `auth.users` table's column set didn't match the live stack's GoTrue-migrated version. In a real deployment the restore target is either the same project (trivially matching GoTrue version) or a freshly provisioned project on the same platform release, so this shouldn't arise — but it needs a real pass against a deployed project before launch.
20. **The M8 rate-limit-audit fix's `dblink` loopback connection was verified end-to-end only against local Supabase's dev `pg_hba.conf`** (M8), which trusts `127.0.0.1` outright as a dev convenience — the fix deliberately avoids relying on that (`inet_server_addr()`, password auth), which should generalize to a real hosted Supabase project's network/auth posture, but this hasn't been confirmed from this sandbox and needs a real pass before launch.
21. **`--label-secondary` (`#7F8C8D` on white ≈3.48:1) falls short of WCAG AA for normal-size text at the footnote/caption sizes it's used at throughout the app** (M8 a11y pass) — a literal value from SPEC's own brand table (§9), not a component-level choice; needs design sign-off on whether the brand table's light-mode value changes, or whether these specific text roles move to a different token, before launch.
